...
const SubmissionRoute = props => {
  ...
  const { submissionsBatches } = useGlobalState();
  const [ownerUid, setOwnerUid] = React.useState();
  const [screenIndex, setScreenIndex] = React.useState();
  const [submissionId, setSubmissionId] = React.useState();
  const [submission, setSubmission] = React.useState();
  const setSubmissionScreen = useSetSubmissionScreen(ownerUid, submissionId);
  React.useEffect(() => {
    const { owner, subId, screen } = qs.parse(location.search);
    if (owner) {
      setOwnerUid(owner);
    }
    if (subId) {
  ...
  const actionsQuery = useActionsQuery(ownerUid, submissionId);
  const actions = useFirestoreCollection(actionsQuery);

  const paymentsQuery = usePaymentsQuery(ownerUid, submissionId);
  ...
  React.useEffect(() => {
    if (submissionsBatches && submissionsBatches[0].length > 0 && submissionId) {
      ...
      if (result) {
        ...
        return listenToSubmission(ownerUid, submissionId).onSnapshot(doc => {
          if (doc.exists) {
  ...
  }, [history, submissionsBatches, submissionId, ownerUid]);
